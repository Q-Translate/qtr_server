...
0 * * * *    www-data    [ -x /usr/local/bin/drush ] && /usr/local/bin/drush @qtr cron > /dev/null 2>&1
9 * * * *    www-data    [ -x /usr/local/bin/drush ] && /usr/local/bin/drush @qtr_dev cron > /dev/null 2>&1
EOF
